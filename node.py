...
class ChordNode:
    ...
    def join(self, bootstrap_ip, bootstrap_port):
        ...
        try:
            ...
            if successor_response["status"] == "success":
                ...
                predecessor_response = self.send_request(bootstrap_ip, bootstrap_port, {"command": "find_predecessor","node_id" : self.node_id})
                print(f"[NODE {self.node_id}] New predecessor get response: {predecessor_response}")
                self.predecessor = predecessor_response["predecessor"]
    ...
    def find_successor(self, node_id):
        ...
        if self.node_id < node_id <= self.successor["node_id"]:
            ...
            return {"status": "success", "successor": {'node_id': self.node_id,'ip': self.ip,'port' : self.port}}
    ...
    def find_predecessor(self, node_id):
        ...
        if self.predecessor["node_id"] < node_id <= self.node_id:
            print(f"[DEBUG] Returning predecessor: {self.predecessor}")
            return {"status": "success", "predecessor": {'node_id': self.node_id,'ip': self.ip,'port' : self.port}}
